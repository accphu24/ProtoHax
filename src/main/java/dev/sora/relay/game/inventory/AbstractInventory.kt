...
private fun slotData(id: Int, slot: Int, netId: Int): ItemStackRequestSlotData {
val type = getSlotTypeFromInventoryId(id, slot)
return ItemStackRequestSlotData(type, slot, netId, FullContainerName(type, null))
}
...
slotData(sourceInfo.first, sourceInfo.second, src.netId),
slotData(destinationInfo.first, destinationInfo.second, dst.netId)
)),
arrayOf(), null
))
} else {
...
val item = content[slot]
                it.requests.add(ItemStackRequest(serverAuthoritative,
                    arrayOf(DropAction(item.count, slotData(info.first, info.second, item.netId), false)),
                    arrayOf(), null
                ))
            }
        } else {
